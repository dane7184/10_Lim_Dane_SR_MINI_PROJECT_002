...
export default function NavbarComponent({ session: propSession }) {
  const pathname = usePathname();
  const [open, setOpen] = useState(false);
  const [showDropdown, setShowDropdown] = useState(false);
  const isAuth = !!propSession;
  const username = propSession?.user?.name || "User";
          ...
          {!isAuth ? (
            ...
            <div className="relative hidden sm:block">
              <button
                ...
                className="flex items-center gap-2 rounded-full bg-lime-100 px-4 py-2 text-sm font-semibold text-gray-900 transition hover:bg-lime-200"
              >
                <div className="flex h-8 w-8 items-center justify-center rounded-full bg-lime-400 font-bold text-gray-900">
              ...
              {showDropdown && (
                <div className="absolute right-0 mt-2 w-40 rounded-xl border border-gray-100 bg-white p-2 shadow-lg">
                  <form action={handleLogout}>
                    <button
                      type="submit"
                      className="w-full rounded-lg px-4 py-2 text-left text-sm font-medium text-red-600 transition hover:bg-red-50"
                    >
                      Sign out
                    </button>
                  </form>
